...
SELECT p.username as username, f.name as faction_name, rc.total_points as total_points, rc.wins as wins, rc.losses as losses, p.steam_id as steam_id, rc.total_points / (rc.wins + rc.losses) as avg, 1 as avatar
FROM players p, factions f, rankings_fps rc
WHERE rc.player_id = p.id AND f.id = rc.faction_id
...
type last_match_commander_record struct {
	...
	ELO           float64 `db:"elo"`
	MatchLength   float64 `db:"match_length"`
...
func update_commmander_leaderboard(db *sqlx.DB, chan_commander_leaderboard chan map[string]interface{}) {
	...
	for {
		go func() {
			...
			err := db.Select(&commanders, commander_rating_leaderboard_query)
			if err != nil {
...
func update_player_leaderboard(db *sqlx.DB, chan_player_leaderboard chan map[string]interface{}) {
	...
	for {
		go func() {
			...
			slog.Info("this is the player_total_scores")
			if err != nil {
				slog.Error(err.Error())
			}
